...
export function ProfileForm({
	...
	user: { name: string; bio: string; avatarUrl: string | null };
}) {
	const [state, formAction] = useActionState(updateProfile, initialState);
	return (
		<form
			action={formAction}
			className="space-y-5 rounded-[1.75rem] border bg-surface p-6 sm:p-8"
		>
			<div>
			...
			<div>
				...
				<input
					...
					placeholder="/images/avatars/your-photo.jpg or https://..."
					className="mt-2 h-12 w-full rounded-2xl border bg-background px-4 text-sm"
				/>
			...
			<div>
				...
				<textarea
					...
					rows={6}
					className="mt-2 w-full rounded-2xl border bg-background px-4 py-3 text-sm leading-6"
				/>
