	...
	return (
		<div>
			<form action={formAction}>
				...
				>
					<HeartIcon className={`size-4 ${state.liked ? "fill-current" : ""}`} />
					{pending ? "Saving..." : state.liked ? "Liked" : "Like"}
				</button>
			</form>
			{state.requiresLogin ? (
				<p className="mt-2 text-xs text-muted"><Link className="font-semibold text-accent" href={`/login?next=/blog/${slug}`}>Log in</Link> to like this story.</p>
			) : null}
		</div>
	);
}
