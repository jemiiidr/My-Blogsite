export function PostCardSkeleton({ featured = false }: { featured?: boolean }) {
	return (
		<div className={`overflow-hidden rounded-[1.75rem] border bg-surface ${featured ? "grid md:grid-cols-[1.2fr_1fr]" : ""}`}>
			<div className={`${featured ? "min-h-72" : "aspect-[16/10]"} animate-pulse bg-surface-muted`} />
